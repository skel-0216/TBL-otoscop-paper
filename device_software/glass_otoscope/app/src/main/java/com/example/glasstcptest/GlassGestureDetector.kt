...
/*
 * This file is derived from GlassGestureDetector.java in Google's
 * glass-enterprise-samples (GestureLibrarySample):
 *   https://github.com/googlesamples/glass-enterprise-samples
 * It was ported to Kotlin and modified for this project; changes from the
 * original are marked with "My Modify" comments.
 */

package com.example.testforglassguesture
...
class GlassGestureDetector(context: Context, private val onGestureListener: OnGestureListener){
    ...
    *   Listeners for the gestures
    ...
    private companion object {
        ...
        // My Modify : add .toLong()
        val TAP_AND_HOLD_THRESHOLD_MS = ViewConfiguration.getLongPressTimeout().toLong()
    ...
    val tapAndHoldCountDownTimer = object : CountDownTimer(TAP_AND_HOLD_THRESHOLD_MS, TAP_AND_HOLD_THRESHOLD_MS){
    ...
    /**
     * init is constructed by usage of this method.
     *
    ...
    /**
     * Passes the MotionEvent object from the activity to the Android link GestureDetector.
     *
    ...
    fun onTouchEvent(motionEvent: MotionEvent) : Boolean{
        ...
        when(motionEvent.action and MotionEvent.ACTION_MASK){
            MotionEvent.ACTION_DOWN -> {
                ...
                // My Modify : split assignments (split the assignment statements)
                firstFingerDownX = motionEvent.x
                firstFingerLastFocusX = motionEvent.x
                firstFingerDownY = motionEvent.y
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (isInTapRegion) {
                    val distance = (firstFingerDistanceX * firstFingerDistanceX) + (firstFingerDistanceY * firstFingerDistanceY)
                    var distanceSecondFinger = 0f
                    if (motionEvent.pointerCount > 1) {
                        distanceSecondFinger = (secondFingerDistanceX * secondFingerDistanceX) + (secondFingerDistanceY * secondFingerDistanceY)
                    }
                ...
                if ((abs(scrollX) >= 1) || (abs(scrollY) >= 1)) {
                    handled = onGestureListener.onScroll(currentDownEvent!!, motionEvent, scrollX, scrollY)
            ...
            MotionEvent.ACTION_UP -> {
                ...
                firstFingerVelocityX = velocityTracker?.getXVelocity(motionEvent.getPointerId(motionEvent.actionIndex)) ?: 0f
                firstFingerVelocityY = velocityTracker?.getYVelocity(motionEvent.getPointerId(motionEvent.actionIndex)) ?: 0f
    ...
    private fun detectGesture() : Boolean {
        ...
        // My Modify add .toDouble for abs()'s result
        val tan : Double = if(firstFingerDistanceX != 0f) abs(firstFingerDistanceY / firstFingerDistanceX).toDouble() else Double.MAX_VALUE

        // My Modify : lift return front of "if"
        return if(isTwoFingerGesture) {
            // My Modify add .toDouble for abs()'s result
            val tanSecondFinger : Double = if(secondFingerDistanceX != 0f) abs(secondFingerDistanceY / secondFingerDistanceX).toDouble() else Double.MAX_VALUE
    ...
    private fun detectOneFingerGesture(tan : Double) : Boolean {
        if (tan > TAN_ANGLE_DEGREES) {
            if (abs(firstFingerDistanceY) < SWIPE_DISTANCE_THRESHOLD_PX || abs(firstFingerVelocityY) < SWIPE_VELOCITY_THRESHOLD_PX) {
                if (isInTapRegion) {
            ...
            if (abs(firstFingerDistanceX) < SWIPE_DISTANCE_THRESHOLD_PX || abs(firstFingerVelocityX) < SWIPE_VELOCITY_THRESHOLD_PX) {
                if (isInTapRegion) {
                    return onGestureListener.onGesture(Gesture.TAP)
                }
