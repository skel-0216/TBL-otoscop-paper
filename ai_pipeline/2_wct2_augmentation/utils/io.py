...
def compute_label_info(content_segment, style_segment):
    # Tensor → numpy (handles GPU tensors too)
    if isinstance(content_segment, torch.Tensor):
        content_segment = content_segment.squeeze().cpu().numpy()
    if isinstance(style_segment, torch.Tensor):
        style_segment = style_segment.squeeze().cpu().numpy()
