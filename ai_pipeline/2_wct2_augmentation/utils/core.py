...
def wct_core_segment(content_feat, style_feat, content_segment, style_segment,
    ...
    def resize(feat, target):
        size = (target.size(2), target.size(1))  # (width, height)
        
        if isinstance(feat, torch.Tensor):
            feat = feat.squeeze().cpu().numpy().astype(np.uint8)  # tensor → numpy
    
        # Grayscale (1 channel)
        if len(feat.shape) == 2:
            return np.asarray(Image.fromarray(feat, mode='L').resize(size, Image.NEAREST))
        
        # Color (3 channels)
        elif len(feat.shape) == 3 and feat.shape[2] == 3:
            return np.asarray(Image.fromarray(feat, mode='RGB').resize(size, Image.NEAREST))
        
        else:
            raise ValueError(f"Unsupported feature shape for resize: {feat.shape}")
